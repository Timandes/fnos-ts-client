...
async function main() {
  ...
  if (!user || !password) {
    console.error(`用法: tsx examples/resource_monitor_general.ts --user <用户名> --password <密码> [-e <服务器地址>]`);
    console.error(`  或: tsx examples/resource_monitor_general.ts --user=<用户名> --password=<密码> [-e=<服务器地址>]`);
  ...
  try {
    ...
    // 调用general方法（使用默认监控项）
    console.log('\n正在调用general方法（使用默认监控项）...');
    try {
      const result = await resourceMonitor.general();
    ...
    console.log('\n正在调用general方法（指定监控项）...');
    try {
      const items = ['storeSpeed', 'netSpeed', 'cpuBusy', 'memPercent'];
      const result = await resourceMonitor.general(10000, items);
      console.log('general响应:');
      console.log(result);
    } catch (e) {
      console.error(`general调用失败: ${e}`);
