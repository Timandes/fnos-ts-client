 ...
 *
 * 运行方式: tsx examples/event_logger.ts --user <用户名> --password <密码> [-e <服务器地址>]
 */

import { FnosClient, EventLogger } from '../src/index.js';
...
async function main() {
  ...
  if (!user || !password) {
    console.error(`用法: tsx examples/event_logger.ts --user <用户名> --password <密码> [-e <服务器地址>]`);
    console.error(`  或: tsx examples/event_logger.ts --user=<用户名> --password=<密码> [-e=<服务器地址>]`);
  ...
  try {
    ...
    const eventLogger = new EventLogger(client);

    // 获取事件日志列表
    console.log('\n=== 事件日志 ===');
    const eventList = await eventLogger.commonList();
    if (eventList.result === 'succ') {
      const data = eventList.data;
      console.log(`总事件数: ${data.total}`);
      console.log(`\n最近 ${data.rows.length} 条事件:`);

      for (const event of data.rows) {
        console.log(`ID: ${event.id}`);
        console.log(`  用户: ${event.username}`);
        console.log(`  时间: ${event.eventtm}`);
